...
class Matrix : public internal::Matrix_base<t_rows, t_cols, Matrix<T_Value, t_rows, t_cols, Layout>, Layout>
{
...
{
	return make_array_of_2d_func_values_impl<decltype(rows), decltype(cols), rows>(func,
...
constexpr auto make_matrix(Func func)
{
	...
	return Matrix<Value, rows, cols>(
		internal::make_array_of_2d_func_values<rows, cols>(func));
}
