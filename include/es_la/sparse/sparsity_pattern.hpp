...
public:
	...
	auto begin()
	{
		return es_util::Zip_iterator(row_indices_.begin(), col_indices_.begin());
	}

	auto end()
	{
		return es_util::Zip_iterator(row_indices_.end(), col_indices_.end());
	}
...
template<class Symmetry_tag> 
es_util::Error Sparsity_pattern<Symmetry_tag>::check() const
{
	es_util::Error err("Sparsity pattern check");
