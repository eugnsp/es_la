...
{
	static_assert(internal::is_vector_expr<Expr1> && internal::is_vector_expr<Expr2>, "Expressions should be vectors");
	static_assert(internal::is_ct_extent_dynamic_or_eq(ct_rows_value<Expr1>, ct_rows_value<Expr2>),
		"Incompatible extents");

	assert(expr1.size() == expr2.size());

	using R = decltype(expr1[0] * expr2[0]);
	R dot{};
...
template<class Expr1, class Expr2,
	typename = std::enable_if_t<std::is_same_v<Value_type<Expr1>, Value_type<Expr2>> &&
								internal::is_fd<Value_type<Expr1>> &&
								internal::is_dynamic_expr<Expr1> && internal::is_dynamic_expr<Expr2>>>
