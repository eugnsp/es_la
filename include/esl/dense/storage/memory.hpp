...
[[nodiscard, gnu::malloc, gnu::alloc_size(1)]] inline void* mem_alloc(const std::size_t size, const std::size_t alignment)
{
	return ::mkl_malloc(size, static_cast<int>(alignment));
}

[[nodiscard, gnu::alloc_size(2)]] inline void* mem_realloc(void* const ptr, const std::size_t size, const std::size_t alignment)
{
...
inline void mem_free(void* const ptr) noexcept
{
	::mkl_free(ptr);
}
} // namespace esl::internal
